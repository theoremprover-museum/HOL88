...
system. This intended to be the first item read by new users of \HOL\ and
...
2.4), which is expected to be found in the directory {\tt Reference/bin}. If
{\t makeindex} is not there, or if it won't run on your machine, you may have
to install a specially-compiled version of {\tt makeindex} in this directory at
your site before building the documentation.  The \TeX\ psfig macros are
required for the fancy cover pages.  See Section~\ref{PS} below for further
information on this.
...
The \HOL\ documentation is designed to be printed either on European A4 sized
...
{\it This section still to be written [tfm].}
...
respectively.  The format of \doc\ files for theorems is the simpler (and most
...
To make it easy to create documentation for theorems, an \ML\ program is
provided which will generate \doc\ files for every axiom, definition, and
theorem in any given theory.  The program is in the file {\tt
...
will (eventually) be a corresponding file called \meta{name}\doc\ in the
...
{\small\verb!\DOC!}, {\small\verb!\TYPE!}, {\small\verb!\SYNOPSIS!},
{\small\verb!\DESCRIBE!}, {\small\verb!\FAILURE!}, {\small\verb!\EXAMPLE!},
{\small\verb!\USES!}, {\small\verb!\COMMENTS!}, {\small\verb!\SEEALSO!} and
{\small\verb!\ENDDOC!}.  The format and content of each of these fields is
...
be misleading as to the nature of the \ML\ function in wuestion.  For example,
...
Most {\small\verb!\TYPE!} fields should to agree precisely with the system
(even up to bracketing and spacing). The shell script
{\small\verb!bin/typecheck!} (written by John Harrison) may therefore be used
...
changes.  The script should have given as an argument the doc-files to be
typechecked, for example `{\tt *.doc}'. The script uses temporary files
{\small\verb!.systypes!} and {\small\verb!.ourtypes!} in the user's home
directory, and does not delete them afterwards.  Here is the script itself:

\medskip
{\small \begin{verbatim}
   (grep -h '\\TYPE' "$@" | sed -e 's/^[^{]*{//; s/}//' |
      tee $HOME/.ourtypes | sed -e 's/:.*/;;/';
    echo 'quit();;') | ../../../hol |
       grep -e '- : ' >$HOME/.systypes
   paste $HOME/.ourtypes $HOME/.systypes |
    sed -e 's/:[^:]*:/:/' | diff $HOME/.ourtypes -
\end{verbatim}}

\medskip
\noindent An example of its use is given below. The output is from {\tt
diff(1)}.
...
{\small\verb!\TYPE!} line, and must be present in every \doc\ file.  It
...
\begin{boxed}\begin{verbatim}
\SYNOPSIS
Specialize the leading universally-quantified variable of a theorem to a
supplied value.
\end{verbatim}\end{boxed}
...
application of the rule or theorem. In the case of a tactic, this may be null,
if the tactic competely solves the goal.
...
order to produce the illustrated effect. Ssometimes this cannot be done clearly
because items above the rule-line are an earlier part of the arguments. For
example, in the case of {\small\verb!AP_THM!} given below, the first argument
is restated.  Terms should be put in quotation marks, and theorems in
parentheses, since this corresponds more closely to an ML expression.
