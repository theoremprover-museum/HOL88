...
let FINITE_CONV = 
    let finE = theorem `sets` `FINITE_EMPTY` in
    let finI = let th1 =  theorem `sets` `FINITE_INSERT` in
	       let th2 = snd(EQ_IMP_RULE (SPECL ["x:*";"s:(*)set"] th1)) in
	           GEN "s:(*)set" (DISCH_ALL (GEN "x:*" (UNDISCH th2))) in
...
let IN_CONV = 
    let check st = assert (\c. fst(dest_const c) = st) in
    let inI = theorem `sets` `IN_INSERT` in
    let inE = GEN "x:*" (EQF_INTRO (SPEC "x:*" th)) where
	      th = theorem `sets` `NOT_IN_EMPTY` in
...
let DELETE_CONV = 
    ...
    let Edel = theorem `sets` `EMPTY_DELETE` in
    let Dins = GENL ["y:*";"x:*"] (SPECL ["x:*";"y:*"] th) where 
      	       th = theorem `sets` `DELETE_INSERT` in
...
let UNION_CONV = 
    let InU  = theorem `sets` `INSERT_UNION` in
    let InUE = theorem `sets` `INSERT_UNION_EQ` in
    let Eu  = CONJUNCT1 (theorem `sets` `UNION_EMPTY`) in
    let check st = assert (\c. fst(dest_const c) = st) in 
    letrec strip_set tm = 
...
let INSERT_CONV = 
    let absth = let th = theorem `sets` `ABSORPTION` in
...
let IMAGE_CONV = 
    let Ith = theorem `sets` `IMAGE_INSERT` and
        Eth = theorem `sets` `IMAGE_EMPTY` in
    let check st = assert (\c. fst(dest_const c) = st) in
