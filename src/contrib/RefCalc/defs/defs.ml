...
% defs.ml - my own abbreviations and definitions %
...
let RHS_RR1 th = 
  CONV_RULE (RHS_CONV (DEPTH_CONV (REWR_CONV th)));;
letrec RHS_RRL thl th =
  if thl = [] then th
  else (RHS_RRL (tl thl)) ((RHS_RR1 (hd thl)) th);;
let RHS_ORR1 th = 
  CONV_RULE (RHS_CONV (ONCE_DEPTH_CONV (REWR_CONV th)));;
...
let RHS_RT1 th = 
  CONV_TAC (RHS_CONV (DEPTH_CONV (REWR_CONV th)));;
let RHS_ORT1 th = 
  CONV_TAC (RHS_CONV (ONCE_DEPTH_CONV (REWR_CONV th)));;
letrec RHS_RTL thl =
  if thl = [] then ALL_TAC
...
let LHS_RR1 th = 
  CONV_RULE (LHS_CONV (DEPTH_CONV (REWR_CONV th)));;
letrec LHS_RRL thl th =
  if thl = [] then th
  else (LHS_RRL (tl thl)) ((LHS_RR1 (hd thl)) th);;
let LHS_ORR1 th = 
  CONV_RULE (LHS_CONV (ONCE_DEPTH_CONV (REWR_CONV th)));;
...
let LHS_RT1 th = 
  CONV_TAC (LHS_CONV (DEPTH_CONV (REWR_CONV th)));;
letrec LHS_RTL thl =
  if thl = [] then ALL_TAC
  else LHS_RT1 (hd thl) THEN LHS_RTL (tl thl);;
let LHS_ORT1 th = 
  CONV_TAC (LHS_CONV (ONCE_DEPTH_CONV (REWR_CONV th)));;
letrec LHS_ORTL thl =
  if thl = [] then ALL_TAC
...
let EX_1PT_CONV tm =
 ...
 let thm = SELECT_RULE (ASSUME tm) in
 let thm1,thm2 = CONJUNCT1 thm,CONJUNCT2 thm in
...
let FORALL_SWAP_CONV tm =
  let x,rest = dest_forall tm in
  let y,rest = dest_forall rest in
  let rh = mk_forall(y,mk_forall(x,rest)) in
  let th1 = DISCH_ALL(GEN y(GEN x(SPEC y(SPEC x(ASSUME tm))))) in
  let th2 = DISCH_ALL(GEN x(GEN y(SPEC x(SPEC y(ASSUME rh))))) in
